...
export default class Index extends Component {
    ...
    render() {
        const successMessage = this.state.successMessage ? (
            <Alert className="alert" variant="success"  dismissible > {this.state.successMessage}</Alert>
        ) : null
                        ...
                        {(this.state.book == null) ?
                                ...
                                {this.state.books.map((book, index) =>
                                    <div key={index} onClick={() => this.handelDetail(book, book.id)}>
                                        ...
                                        <Card className="card"  key={book.id}>
                                            ...
                                            <Card.Body >
                                            <Card.Title className="Card.Title">{book.bookName}</Card.Title>
                                            {/* <div className="body-card"></h4></div> */}
                                            </Card.Body>
                                        </Card>
                                    </div>
                            ...
                            <div>
                                {(this.state.isEdit === true) ?
                                    ...
                                    :
                                    <Detail book={this.state.book} editView={this.editView} key={this.state.book.id} deleteBook={this.deleteBook} isEdit={this.state.isEdit} user={this.props.user} userToken={this.props.userToken} loadBook={this.loadBook} />
                                }
                            </div>
